# service-specs

> 제품을 **무엇을·왜·누구에게** 만드는지. 기능별로 나눈다. 작업 원칙은 루트 [`CLAUDE.md`](../../CLAUDE.md).

| 문서 | 언제 보는가 |
|---|---|
| [`overview.md`](./overview.md) | 제품의 목적·대상·범위·지표가 궁금할 때 |
| [`auth.md`](./auth.md) | 로그인·빠른 잠금해제(PIN)를 다룰 때 |
| [`transactions.md`](./transactions.md) | 거래 입력·내역·월간 요약을 다룰 때 |
| [`categories.md`](./categories.md) | 카테고리(분류)를 다룰 때 |
| [`payment-methods.md`](./payment-methods.md) | 결제수단(카드/계좌)을 다룰 때 |
| [`recurring.md`](./recurring.md) | 정기 거래와 도래 처리를 다룰 때 |
| [`export.md`](./export.md) | 데이터 내보내기(백업)를 다룰 때 |
