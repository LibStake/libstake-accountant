...
test.describe("유동성", () => {
  ...
  test("정기 지출만 있으면 필요 보유액과 적자 경고를 보인다", async ({ page, appUser }) => {
    ...
    await page.goto("/liquidity");
    await expect(page.getByText("지금 필요한 최소 보유액")).toBeVisible();
    await expect(page.getByText("정기 수입이 지출을 못 메워요")).toBeVisible();
  });
