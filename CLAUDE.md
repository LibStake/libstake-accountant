...
기준: 다른 세션에서 해당 주석을 단어를 포함하여 온전히 이해할 수 있는가? 및 주석이 대화형으로 작성되지 않았는가? 
...
- **코드 첨가 금지**: 템플릿 코드를 제외한 어떠한 코드, 변수, 상수 명, DB 내 테이블 및 컬럼 명 등 코드에 해당하는 부분을 작성하지 않는다. SOT은 코드 그 자체이지 문서가 아니다. 문서는 방향성 만을 서술한다.
...
### 0.10 Root 프로젝트 내 CLAUDE.md 수정 금지
사용자에 의한 명백한 정책 변경이 발생하는 경우가 아니면 이 분서내의 어떠한 내용도 수정하지 않는다. 이 문서는 대원칙이다.